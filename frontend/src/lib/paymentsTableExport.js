...
// The complete roster export: every enrolled person (not just those who've
// shown up yet), with the registration fee, the participation amount paid,
// and their combined total — landscape, like the inscritos list, since it
// carries an extra column.
export function buildFullPaymentsTablePdf({ title, subtitle, rows, totalInscription, totalParticipation, totalGeneral, formatAmount }) {
  ...
  const columns = [
    { key: '_idx', label: '#', width: 30 },
    { key: 'full_name', label: 'Nom complet', width: 220 },
    { key: 'vendeur_name', label: 'Vendeur', width: 140 },
    { key: 'inscription_fee', label: "Frais d'inscription (HTG)", width: 140 },
    { key: 'participation_paid', label: 'Frais de participation (HTG)', width: 150 },
    { key: 'total', label: 'Total (HTG)', width: 110 },
  ];
  ...
  pdf.setFontSize(11);
  pdf.setTextColor(30, 41, 59);
  pdf.text(`Total inscription: ${formatAmount(totalInscription)} HTG`, margin, y);
  y += 18;
  pdf.text(`Total participation: ${formatAmount(totalParticipation)} HTG`, margin, y);
  y += 18;
  pdf.text(`Total général: ${formatAmount(totalGeneral)} HTG`, margin, y);
